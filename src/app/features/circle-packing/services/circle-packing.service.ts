...
export class CirclePackingService {
  render(config: {
    ...
    width: number;
    height: number;
    onNodeClick: (node: CirclePackingNode, d: HierarchyCircularNode<CirclePackingNode>) => void;
  }) {
    const svg = select(config.svgElement);
    svg.selectAll('*').remove();

    const hierarchyData: CirclePackingNode = this.buildHierarchy(config.data, config.type);

    const root = hierarchy<CirclePackingNode>(hierarchyData)
      .sum(d => d.value ?? 0)
      .sort((a, b) => (b.value ?? 0) - (a.value ?? 0));

    const packedRoot = pack<CirclePackingNode>().size([config.width, config.height]).padding(5)(
      root,
    );
  ...
  private handleMouseOver(
    ...
    selection.on('mouseover', function () {
      ...
      select(this).attr('stroke', '#000').attr('stroke-width', 2).attr('fill', lighter);
    });
  }
  ...
  private handleMouseOut(
    ...
    selection.on('mouseout', function () {
      const d = select(this).datum() as HierarchyCircularNode<CirclePackingNode>;
      const baseColor = d.children
        ? lightenColor(countriesColor(d.data.name), 40)
        : countriesColor(d.parent?.data.name || '');
      select(this).attr('stroke', '#444').attr('stroke-width', 1).attr('fill', baseColor);
    });
  }

  private buildHierarchy(data: Continent, type: 'population' | 'land_area_km2'): CirclePackingNode {
    return {
      name: 'Europe',
